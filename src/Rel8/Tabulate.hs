...
import qualified Opaleye.Internal.PrimQuery as Opaleye hiding ( aggregate )
...
import Rel8.Query.Exists ( exists, whereExists, whereNotExists )
import Rel8.Query.Filter ( where_ )
import Rel8.Query.List ( catNonEmptyTable )
import Rel8.Query.Maybe ( optional )
import Rel8.Query.Opaleye ( mapOpaleye, zipOpaleyeWith )
...
import Rel8.Table.Maybe ( MaybeTable( MaybeTable ), justTable, maybeTable )
...
alignWith f (Tabulation as) (Tabulation bs) = Tabulation $ \p -> do
  tkab <- uncurry TheseTable <$> liftF2 (zipOpaleyeWith fullOuterJoin) as bs p
  let
  ...
  where
    fullOuterJoin a b =
      Opaleye.joinExplicit
        (keyed unpackspec unpackspec)
        (keyed unpackspec unpackspec)
        justTable
        justTable
        Opaleye.FullJoin
        a
        b
        (toColumn . toPrimExpr . condition)
      where
        condition ((k, _), (k', _)) = fromMaybe true (liftA2 (==:) k k')
    recover (TheseTable mma@(MaybeTable _ ma) mmb@(MaybeTable _ mb)) = do
      a <- ma
      b <- mb
      case a <$ mma <|>: b <$ mmb of
